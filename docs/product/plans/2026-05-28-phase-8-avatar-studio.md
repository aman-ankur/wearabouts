...
- Avatar render prompt upgraded to `avatar-studio-v1.5` for balanced studio realism plus a slightly stronger fashion-catalog lift around pose, face focus, neck posture, shoulder set, camera, grooming, and garment-fit direction while explicitly avoiding jaw sculpting, body reshaping, filters, and model-makeover drift. The real provider now sends the body reference before the face reference so the body image anchors head scale, neck, shoulders, posture, and skin/lighting continuity; the prompt also explicitly rejects pasted-on face composition. When a saved look includes outerwear but no top, the prompt allows one modest neutral inner layer so jackets and suits are never rendered over a bare torso.
