...
try {
  ...
  await screenshot(client, "wearabouts-ux-phase4-real-upload.png");

  await navigate(client, `${appUrl}/processing/manual-job?batchId=manual-batch`);
  await waitFor(client, `() => document.body.innerText.includes('This upload needs another try')`);
  await screenshot(client, "wearabouts-ux-phase4-processing-failed-no-env.png");

  console.log("Captured Phase 4 real-mode screenshots.");
} finally {
  client.close();
}
